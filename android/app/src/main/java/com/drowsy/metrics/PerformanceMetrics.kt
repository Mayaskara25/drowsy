...
class PerformanceMetrics {
    ...
    private var framesIn = 0L; private var framesOut = 0L
    private var lastTick = System.currentTimeMillis()
    private var inferCount = 0; private var inferMsSum = 0L
    ...
    fun onDropped() { _snapshot.value = _snapshot.value.copy(droppedFrames = _snapshot.value.droppedFrames + 1) }
    fun onNetworkBytes(bytes: Long, dtMs: Long) {
        if (dtMs > 0) _snapshot.value = _snapshot.value.copy(networkKbps = bytes * 8f / dtMs)
    }

    private fun maybeTick(faceConf: Float, tq: Float, score: Int, state: String) {
        val now = System.currentTimeMillis(); val dt = now - lastTick
        if (dt >= 1000) {
            ...
            _snapshot.value = PerfSnapshot(
                ...
                fatigueScore = score, state = state, droppedFrames = _snapshot.value.droppedFrames,
                networkKbps = _snapshot.value.networkKbps
            )
